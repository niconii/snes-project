...
_fast           rep #%11101011      ; set status flags
                ...
                ;; Screen settings
                lda #$8f            ; turn off screen, max brightness
                sta INIDISP
                ...
                ;; VRAM settings
                lda #$80            ; auto-inc, no translation, step by 1
                sta VMAIN
                ...
                lda #$ff            ; joypad IO pins floating
                sta WRIO
...
-               stz DMAP0,x         ; A -> B, HDMA direct, increment A, [B]
                ...
                stz UNUSED0,x       ; clear unused byte
                clc
                adc #$10
                tax
                dey
                ...
                ;; Initialize OAM
                ldx #%00001010      ; A -> B, fixed, write to [B, B]
                stx DMAP0
                ldx #<OAMDATA       ; B-bus addr = OAMDATA
                stx BBAD0
                lda #$0220          ; bytes to transfer = $220
                sta DAS0L
                sty MDMAEN          ; clear OAM

                ;; Initialize CGRAM
                ldx #<CGDATA        ; B-bus addr = CGDATA
                stx BBAD0
                lda #$0200          ; bytes to transfer = $200
                sta DAS0L
                sty MDMAEN          ; clear CGRAM

                ;; Initialize VRAM
                ldx #%00001001      ; A -> B, fixed, write to [B, B+1]
                stx DMAP0
                ldx #<VMDATAL       ; B-bus addr = VMDATAL
                stx BBAD0
                sty MDMAEN          ; clear VRAM

                ;; Initialize WRAM
                ldx #%00001000      ; A -> B, fixed, write to [B]
                stx DMAP0
                ldx #<WMDATA        ; B-bus addr = WMDATA
                stx BBAD0
